...
{
while(cumsum[j]>=gbp1[count-1]+chunks/(1+overlap))	//add a new_ chunk
{gchr[count]=chr[j];gbp1[count]=gbp1[count-1]+chunks/(1+overlap);gbp2[count]=gbp1[count]+chunks;count++;}
}
else	//new_ chr - make new_ chunk (or two)
{
...
if(chr[j]==gchr[count-1])	//same chromosome
{
while(bp[j]>gbp1[count-1]+chunksbp/(1+overlap))	//add a new_ chunk
...
int num_parts, gene_perms, start, end, new_, new2, *usedpreds;
...
//each new_ pvalue is frac*(1-Igamma(stat*beta, alpha))
for(g=0;g<num_genes;g++)	
{
if(gdetails[g]>0)	//has length>0
{
...
while(new_>=0)
{
if(gchr[new_]!=gchr[g]){break;}	//(definitely) out of range
if(gends[new_]>=start&&pvalues[new_]!=-9999)	//touches, so consider including
{
if(gdetails[new_]==0)	//include automatically - hopefully few of these
{
if(gstarts[new_]<start){start=gstarts[new_];startbp=gbp1[new_];}
new2=new_;
}
else	//include if significant
{
if(pvalues[new_]<=cut2)
{
if(gstarts[new_]<start){start=gstarts[new_];startbp=gbp1[new_];}
new2=new_;
}}
...
new_=g+1;new2=g;
while(new_<num_genes)
{
if(gchr[new_]!=gchr[g]){break;}	//(definitely) out of range
if(gstarts[new_]<=end&&pvalues[new_]!=-9999)	//touches, so consider including
{
if(gdetails[new_]==0)	//include automatically - hopefully few of these
{
if(gends[new_]>end){end=gends[new_];endbp=gbp2[new_];}
new2=new_;
}
