...
{
if(scoretest==0)	//get thetas (no missing)
{
...
if(num_prs>0)	//update Z for first chromosome, resolve null model
{
...
if(adjpreds==1)	//fill Z3 with WZ, then get Z2 = Z inv(ZTZ3) - do separately for each phenotype
{
for(m=0;m<num_resps_use;m++)
{
...
{
if(chr[bitstart]!=chrindex3[mark3])	//resolve null model and maybe update Z2/Z3
{
...
if(mults[bitstart+j]!=-9999)	//will be testing - remember to scale theta and SD by Pscales and Pscales2 (not necessary for scoretest=0)
{
...
stats[1+mark]=pow(XTCX[j],-.5)*Pscales2[m];
stats[2+mark]=stats[0+mark]/stats[1+mark];
...
if(adjpreds==1)	//revisit the most significant predictors (must have scoretest=1)
{
for(j=0;j<bitlength;j++)
{
mark=4*j;
...
if(spatest==1)	//compute spa test statistic - remember to scale by Pscales3
{
...
if(spaside==1){spastatus[j]=spa_logistic_one(YTdata[j+m*bitlength], data+(size_t)j*num_samples_use, num_samples_use, nullprobs, nullweights, stats+mark, Pscales3[m]);}
else{spastatus[j]=spa_logistic_two(YTdata[j+m*bitlength], XTCX[j], data+(size_t)j*num_samples_use, num_samples_use, nullprobs, nullweights, stats+mark, Pscales3[m]);}
}
}
...
free(Pscales);free(Pscales2);free(Pscales3);
