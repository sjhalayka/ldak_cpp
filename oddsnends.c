...
while(1)
{
_try=lower+(upper-lower)/2;

if(fraction<cumsum[_try])	//move left
{
upper=_try;
_try=lower+(upper-lower)/2;
}

if(fraction>=cumsum[_try+1])	//move right
{
lower=_try;
_try=lower+(upper-lower)/2;
}

if(fraction>=cumsum[_try]&&fraction<cumsum[_try+1]){break;}

count++;
if(count>100){printf("Error, can not find %f out of %f with %d - last _try %d boundaries %f %f\n\n", fraction, cumsum[length], length, _try+1, cumsum[_try], cumsum[_try+1]);exit(1);}
}

if(flag==1)	//move down values
{
value=cumsum[_try+1]-cumsum[_try];
for(j=_try;j<length;j++){cumsum[j+1]-=value;}
}
