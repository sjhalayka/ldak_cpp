...
//read loco.details for all phenotypes - want sample sizes, scaling and root of scaling, and effect calibration - and check prs files
...
//skip three rows, and get calibration, then integrate this into Plambdas and Proots
...
{printf("Error reading %s; Row 7 should begin \"Effect_Calibration\" (not %s), suggesting the file has been changed since creation with \"--ridge\", \"--bolt\", \"--bayesr\" or \"--elastic\"\n\n", filename, readstring);exit(1);}
Plambdas[m]*=readdouble;
Proots[m]*=readdouble;
...
for(j=0;j<num_prs;j++)	//add in genome-wide covarprs
{
for(i=0;i<num_samples_use;i++){Z[i+(num_fixed-num_prs+j)*num_samples_use]=covprs[(size_t)j*num_samples_use+order[i]];}
}

//decide how much to scale effect sizes, sds and spa test statistics (separate to lambdas and roots)
for(m=0;m<num_resps_use;m++)
{
if(mode==131||fastgwa==0)	//using linear model
{
...
else		//get thetas, nullprobs, nullweights and W x adjusted response, padding missing
{
...
if(num_prs>0)	//update Z for first chromosome covarprs, resolve null model
{
...
if(num_prs>0)	//necessary to adjust prs for covarprs
{
for(m=0;m<num_resps_use;m++)
{
if(mode==131){reg_covar_matrix(prs+(mark2+m*num_chr2)*num_samples_use, covprs+(size_t)(1+mark3)*num_prs*num_samples_use, num_samples_use, 1, num_prs);}
else{reg_covar_weighted(prs+(mark2+m*num_chr2)*num_samples_use, covprs+(size_t)(1+mark3)*num_prs*num_samples_use, num_samples_use, 1, num_prs, nullweights+m*num_samples_use);}
}
}

for(m=0;m<num_resps_use;m++)
{
if(mode==131)	//adjusted phenotype is Yadj2 = Yadj - PRS, adjust for covariates, get variance of Yadj2
{
for(i=0;i<num_samples_use;i++){Yadj2[i+m*num_samples_use]=Yadj[i+m*num_samples_use]-prs[order[i]+(mark2+m*num_chr2)*num_samples_use];}
...
else	//adjusted phenotype is Yadj2 = Yadj - W PRS, get variance of 1/root(W) Yadj2 (variance redundant if fastgwa!=0)
{
...
{
if(chr[bitstart]!=chrindex3[mark3])	//resolve null model and maybe update Z2/Z3, plus more things if fastgwa!=0
{
...
if(mults[bitstart+j]!=-9999)	//will be testing - remember Pscales and Pscales2 and Plambdas and Proots
{
...
stats[1+mark]=pow(value2,.5)*Pscales2[m]*Proots[m];
stats[2+mark]=stats[0+mark]/stats[1+mark];
...
if(spatest==1)	//compute spa test statistic - remember Pscales3 and Proots
{
...
if(mode==131||fastgwa==0){spastatus[j]=spa_test(YTdata[j+m*bitlength], data+(size_t)j*num_samples_use, num_samples_use, num_knots, knots+m*num_knots, num_bins, bins, CGF0+m*num_knots, CGF1+m*num_knots, CGF2+m*num_knots, CGF3+m*num_knots, stats+mark, Pscales3[m]*Proots[m]);}
else
{
if(spaside==1){spastatus[j]=spa_logistic_one(YTdata[j+m*bitlength], data+(size_t)j*num_samples_use, num_samples_use, nullprobs2+m*num_samples_use, nullweights2+m*num_samples_use, stats+mark, Pscales3[m]*Proots[m]);}
else{spastatus[j]=spa_logistic_two(YTdata[j+m*bitlength], XTCX[j], data+(size_t)j*num_samples_use, num_samples_use, nullprobs2+m*num_samples_use, nullweights2+m*num_samples_use, stats+mark, Pscales3[m]*Proots[m]);}
}
}
...
free(Pincs);free(Plambdas);free(Proots);free(hers);
